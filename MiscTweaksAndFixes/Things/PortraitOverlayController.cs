...
namespace MiscTweaksAndFixes.Things
{
    internal partial class PortraitOverlay : ViewBase<PartyCharacterVM>, IDisposable
    {
        ...
        private class Patches
        {
            ...
            private static void OnInitialize<TBuffView>(PartyCharacterView<TBuffView> __instance)
                where TBuffView : ViewBase<UnitBuffPartVM>
            {
                //var po = new PortraitOverlay();

                //__instance.AddDisposable(po);
                
                //po.SetupPortraitOverlay(__instance,
                ...
                where TBuffView : ViewBase<UnitBuffPartVM>
            {
                ...
                var po = __instance.m_BindDisposable.OfType<PortraitOverlay>().FirstOrDefault();
        ...
        private PartyCharacterVM? VM
        {
            ...
            set
            {
                if (value != null)
                {
                    MicroLogger.Debug(() => $"Binding to {value.CharacterName} ViewMode");
